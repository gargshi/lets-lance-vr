			...
			<div>
				<div className={`grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-4`}>
					{currentMessages.map((msg) => (						
						<MessageBlock key={msg.id} msg={msg} handleDelete={handleDelete} sevClr={sevClr} sevClrBdr={sevClrBdr} timeAgo={timeAgo} />
					))}
				</div>				
			</div>
			<Paginator currentPage={currentPage} totalPages={totalPages} handlePrev={handlePrev} handleNext={handleNext} />
...
const MessageBlock: React.FC<{ msg: Message; handleDelete: (id: number, type: string) => void; sevClr: Record<string, string>;sevClrBdr: Record<string, string>; timeAgo: (dateString: string) => string }> = ({ msg, handleDelete, sevClr, sevClrBdr, timeAgo }) => {
	const [revealMsg, setRevealMsg] = useState(false); // State to track if the message should be revealed
	return (
			<>
				<div 
					...
					key={msg.id}
					>
					<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2" onClick={() => setRevealMsg(!revealMsg)}>
						<div className="flex flex-col sm:flex-row sm:items-center gap-4">
							...
							<div className="flex flex-col">
								<div className={`text-xs relative`}>
									<p>{(msg.content.length > 40 && !revealMsg) ? msg.content.slice(0, 40) + '...' : msg.content}</p>
								</div>								
							</div>
						</div>
